...
def test_write_release_checksums_records_each_asset(tmp_path: Path) -> None:
    windows_asset = tmp_path / "hunterX_windows_0.4.6.zip"
    source_asset = tmp_path / "hunterX_source_0.4.6.zip"
    manifest = tmp_path / "SHA256SUMS_v0.4.6.txt"
    ...
    assert "hunterX_windows_0.4.6.zip" in contents
    assert "hunterX_source_0.4.6.zip" in contents
...
def test_write_release_checksums_fails_closed_for_missing_asset(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="Release asset is missing"):
        write_checksums([tmp_path / "missing.zip"], tmp_path / "SHA256SUMS_v0.4.6.txt")
...
def test_build_source_archive_verifies_exact_git_commit(tmp_path: Path) -> None:
    ...
    archive = build_source_archive(
        version="0.4.6",
        output=tmp_path / "hunterX_source_0.4.6.zip",
        repo_root=repo,
        commit=commit,
    )
