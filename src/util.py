...
def _build_http_adapter() -> HTTPAdapter:
    ...
    return HTTPAdapter(max_retries=retry, pool_connections=10, pool_maxsize=20)


def get_http_session() -> requests.Session:
    """Return a shared requests session with conservative retry defaults."""
...
def close_http_session() -> None:
    """Close the shared HTTP session, primarily for tests and clean shutdown."""
    global _HTTP_SESSION
    with _HTTP_SESSION_LOCK:
...
def http_request(
    ...
    **kwargs,
    ...
    """Make an HTTP request through the shared session with a mandatory timeout."""
    if timeout is None or timeout <= 0:
        raise ValueError("HTTP timeout must be a positive number")

    client = session or get_http_session()
    return client.request(method, url, timeout=timeout, **kwargs)


def http_get(url: str, *, timeout: float = DEFAULT_HTTP_TIMEOUT, **kwargs) -> requests.Response:
    return http_request("GET", url, timeout=timeout, **kwargs)


def http_post(url: str, *, timeout: float = DEFAULT_HTTP_TIMEOUT, **kwargs) -> requests.Response:
    return http_request("POST", url, timeout=timeout, **kwargs)
...
def save_url_to_file(remote_url, CONST_MAXBOT_ANSWER_ONLINE_FILE, force_write = False, timeout=0.5):
    ...
    if len(remote_url) > 0:
        html_result = None
        try:
            html_result = http_get(remote_url, timeout=timeout, allow_redirects=False)
        except Exception as exc:
            html_result = None
            #print(exc)
...
def get_target_index_by_mode(list_length, auto_select_mode):
    ...
    elif mode == CONST_MOST_REMAINING:
        return 0
...
def kktix_get_registerStatus(event_code):
    ...
    try:
        html_result = http_get(url, headers=headers, timeout=0.7, allow_redirects=False)
    except Exception as exc:
        html_result = None
...
def ibon_fetch_and_parse_livemap(performance_id, debug=None):
    ...
    try:
        resp = http_get(url, headers={"User-Agent": USER_AGENT}, timeout=3)
