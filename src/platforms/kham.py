...
async def nodriver_kham_date_auto_select(tab, domain_name, config_dict):
    ...
    # Keep this light; the outer loop will retry quickly while the page settles.
    ...
    if not is_date_assign_by_bot and auto_reload_coming_soon_page_enable:
        if is_coming_soon or formated_area_list is None or len(formated_area_list) == 0:
            try:
                ...
                await _reload_page_when_due(tab, config_dict, "kham_date_reload", "[KHAM DATE]")
            except Exception as exc:
                debug.log("Auto reload exception:", exc)
    elif is_date_assign_by_bot:
        _state["kham_date_reload_next_at"] = 0
...
async def nodriver_kham_performance(tab, config_dict, ocr, domain_name, model_name):
    ...
    if is_need_refresh:
        ...
        await _reload_page_when_due(tab, config_dict, "kham_area_reload", "[KHAM PERFORMANCE]")
    ...
    if is_price_assign_by_bot and 'udnfunlife' not in domain_name:
        for _ in range(20):  # up to ~2s; exits as soon as the navigation is seen
            ...
            (function() {{
                ...
                allSeats.forEach(seat => {{
                    ...
                    if (style.includes('icon_chair_empty_1.gif')) {{
                        const title = seat.getAttribute('title');
                        if (title && title.includes('排') && title.includes('號')) {{
                            // Parse seat info: {{area}}-{{row}}排-{{seat}}號
                            const parts = title.split('-');
                            if (parts.length >= 3) {{
