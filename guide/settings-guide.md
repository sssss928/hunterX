...
在指定時間到達前，程式會持續等待並顯示倒數；時間到達後立即重整頁面開始搶票。新版支援毫秒格式，也可搭配延遲校準刷新。

**格式**：YYYY/MM/DD HH:MM:SS 或 YYYY/MM/DD HH:MM:SS.SSS（24小時制）

**範例值**：

- 2025/12/25 14:00:00
- 2025/12/25 13:59:59.400
...
**延遲校準刷新（refresh_calibration）**：

啟用後建議在 `refresh_datetime` 填「官方開賣時間」，例如 `2025/12/25 14:00:00.000`，不要先自行扣掉延遲。HunterX 會用下列公式推算實際刷新觸發時間，並在設定頁顯示結果：
...
| 欄位 | 預設 | 說明 |
|------|------|------|
| 時間來源網址 | https://time.is/ | 可輸入公開國際時鐘或時間 API URL；按「自動校準」後 HunterX 會讀取 HTTP `Date` 標頭或常見 JSON 時間欄位 |
| 售票網址 | 空白 | 可輸入活動頁或售票首頁；留空時估算按鈕會使用上方首頁欄位 |
| 自動更新校準值 | 關閉 | 設定頁開啟時每 1 秒重新校準一次；每秒自動校準使用 1 個輕量樣本，手動校準使用多樣本 |
| 校時偏差(ms) | 0 | 本機時間相對標準時間的偏差；Windows/NTP 已同步時通常接近 0 |
| 前端處理(ms) | 45 | 瀏覽器排程、事件處理、畫面幀/動畫等待、開始 reload 的本機耗時估計；可按「估算動畫/前端延遲」自動帶入 |
| 估計單向網路延遲(ms) | 35 | 用售票網址少量 HTTP RTT 粗估，近似取 RTT/2；不是精準上行延遲 |
| 安全保留(ms) | 30 | 避免太早送達的保留值，數字越大越接近開賣後才觸發 |
| 凍結秒數 | 10 | 進入最後 N 秒後固定已算出的觸發時間，避免最後一秒因人工修改或量測抖動跳來跳去 |

**自動校準怎麼用**：

1. 在 `refresh_datetime` 填官方開賣時間，例如 `2025/12/25 14:00:00.000`。
2. 開啟延遲校準刷新。
3. 時間來源網址可先用預設 `https://time.is/`，或改成你信任的公開時間 API。
4. 售票網址可留空使用上方首頁/活動網址，或貼入該場活動頁。
5. 按「一鍵估算全部」，HunterX 會估算校時偏差、售票站估計單向網路延遲與本機前端/動畫延遲並自動帶入欄位。
6. 觀察推算刷新觸發時間；需要更保守時只調整「安全保留(ms)」。

**倒數顯示**：未到觸發時間時顯示「距離觸發」。到點後固定顯示 `0.000s`，並提示後續交由「自動刷新頁面間隔(秒)」流程繼續刷新，不再顯示負倒數。

**時間來源限制**：以 Windows 系統時間搭配 NTP 同步作為基準仍然最穩。Time.is 或懸浮時鐘可用來觀察本機時間是否偏移，但 HunterX 自動校準讀取的是該網址回應的 HTTP `Date` 或 JSON 時間，不是讀網頁畫面上的大時鐘。該延遲也只代表你到該時間來源的路徑，不等於拓元售票伺服器路徑。

**售票網址估算限制**：售票網址估算只做少量 HTTP HEAD/GET 樣本，用來估 RTT、抖動與估計單向網路延遲。它不能預知伺服器排隊、驗證碼、庫存競爭、頁面跳轉、實際按鈕動畫或表單送出後的伺服器處理時間，也不能量到精準上行延遲。

**限制**：最後一秒突然發生網路壅塞時，任何提前計算都無法預知。凍結結果是為了避免臨場數值亂跳；不是保證命中。若請求太早導致頁面仍是倒數狀態，仍可能需要人工再刷新一次。
